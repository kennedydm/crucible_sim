...
mutable struct SimModel
    ...
    function SimModel(config::SimConfig, rng::Xoshiro)
        ...
        ret.gov_velocity = (rand(rng) - 0.5) * 2 * config.dyn_model_gov_vel_mag / 1000.0
        return ret
    end
end
...
function model_query(mdl::SimModel, x_in::Float64, iter::Int)
    ...
    u = spline_eval(mdl.gov_spline, x)
    u = sim_fract(u + mdl.gov_velocity * iter)    # wrap [0,1)
